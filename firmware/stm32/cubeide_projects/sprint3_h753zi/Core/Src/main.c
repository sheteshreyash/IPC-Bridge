...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, LD1_GREEN_Pin|DATA_READY_Pin|LD3_RED_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(LD2_YELLOW_GPIO_Port, LD2_YELLOW_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : LD1_GREEN_Pin DATA_READY_Pin LD3_RED_Pin */
  GPIO_InitStruct.Pin = LD1_GREEN_Pin|DATA_READY_Pin|LD3_RED_Pin;
...
void StartLedTask(void *argument)
{
  ...
  for(;;)
  {
	  // Toggle Green OS Heartbeat every 500ms (1 Hz)
